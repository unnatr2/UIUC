...
// The following program uses trees to create an optimized floorplan of a circuit.
// The program inputs the various modules from a file and initializes them into a
// left-skewed tree. The program then uses various optimization techniques to find
// the smallest possible area of the floorplan. Finally, the program converts the
// tree into a post-order traversal of the floorplan.

#include "floorplan.h"

int same(node_t* a,node_t* b);

// Global variables.
int num_modules;                                            // Number of input modules
...
int is_leaf_node(node_t* ptr) {
  if (ptr->left==NULL && ptr->right==NULL){                 // Check if node has left or right nodes
    return 1;                                               // Return 1 if they are NULL
  }
...
int is_internal_node(node_t* ptr) {
  if (ptr->cutline==V || ptr->cutline==H){                  // Check if node has cutline
    return 1;                                               // Return 1 if V or H
  }
  return 0;                                                 // Else, return 0
}
...
int is_in_subtree(node_t* a, node_t* b) {
  return a!=NULL && ( same(a,b) || is_in_subtree(a->left,b) || is_in_subtree(a->right,b));      // Check if same or in branches of a
}

// Function: same
// Return 1 if trees rooted at 'a' and 'b' are same or not. Else, 0.
int same(node_t* a,node_t* b){
  if (a==NULL && b==NULL){                                  // Check if both trees are empty
    return 1;                                               // Return 1
  }
  if (a==NULL || b==NULL) {                                 // Check if only one tree is empty
    return 0;                                               // Return 0
  }
  // Check if the nodes have the same module data and the same cutline data. Then check if left branch is same and right branch is same.
  // Then return
  return ((a->module==b->module) && (a->cutline==b->cutline) && same(a->left,b->left) && same(a->right,b->right));
}
// Procedure: rotate
...
void rotate(node_t* ptr) {
  ...
  temp=ptr->module->w;                                      // Set temp to width of module
  ptr->module->w=ptr->module->h;                            // Set width of module to height of module
  ptr->module->h=temp;                                      // Set height of module to temp
}
...
void recut(node_t* ptr) {
  if(!is_internal_node(ptr)) return;                        // Check if internal node
  assert(ptr->module == NULL && ptr->cutline != UNDEFINED_CUTLINE);
  if (ptr->cutline==V){                                     // Check if cutline is V
    ptr->cutline=H;                                         // Change cutline to H
    return;                                                 // Return
  }
  ptr->cutline=V;                                           // Change cutline to V
  return;                                                   // Return
}
...
void swap_module(node_t* a, node_t* b) {
  if(!is_leaf_node(a) || !is_leaf_node(b)) return;          // Return if leaves
  ...
  module_t *mod_temp;                                       // Declare variable
  mod_temp=a->module;                                       // Set mod_temp to module of a
  a->module=b->module;                                      // Set module of a to module of b
  b->module=mod_temp;                                       // Set module of b to mod_temp
}
...
void swap_topology(node_t* a, node_t* b) {
  if(a == NULL || b == NULL) return;                        // Return if a or b are NULL
  if(a->parent == NULL || b->parent == NULL) return;        // Return if a or b are roots
  if(is_in_subtree(a, b) || is_in_subtree(b, a)) return;    // Return if a is a subtree of b or b is a subtree of a
  assert(a->parent != NULL && b->parent != NULL);           // Make sure a and b are not roots
  char leaf_a, leaf_b;                                      // Declare variables
  if (a->parent->left==a){                                  // If a is the left of parent
    leaf_a='L';                                             // Save leaf_a as left
  }
  else {
    leaf_a='R';                                             // Save leaf_a as right
  }
  if (b->parent->left==b){                                  // If b is the left of parent
    leaf_b='L';                                             // Save leaf_b as left
  }
  else {
    leaf_b='R';                                             // Save leaf_b as right
  }
  node_t *par_a=a->parent, *par_b=b->parent;                // Declare variables
  node_t *save_a=a, *save_b=b;                              // Declare variables
  a->parent=par_b;                                          // Make parent of a, parent of b
  b->parent=par_a;                                          // Make parent of b, parent of a
  if (leaf_a=='L'){                                         // If a is left
    par_a->left=save_b;                                     // Set b as left of parent of a
  }
  else {
    par_a->right=save_b;                                    // Set b as right of parent of a
  }
  if (leaf_b=='L'){                                         // If b is left
    par_b->left=save_a;                                     // Set a as left of parent of b
  }
  else {
    par_b->right=save_a;                                    // Set a as right of parent of b
  }
}
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  if (ptr == NULL) return;                                  // Return if pointer is NULL
  postfix_traversal(ptr->left,nth,expression);              // Traverse left
  postfix_traversal(ptr->right,nth,expression);             // Traverse right
  if (ptr->cutline==UNDEFINED_CUTLINE){                     // Check if leaf
    expression[*nth].module=ptr->module;                    // Save module to expression
    (*nth)++;                                               // Move to next index
    return;                                                 // Return
  }
  expression[*nth].cutline=ptr->cutline;                    // Save internal node to expression
  (*nth)++;                                                 // Move to next index
  return;                                                   // Return
}
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  assert(n >= 0 && n < num_modules);                        // Make sure n is between 0 and num_modules
  node_t *temp_cutline=(node_t*)malloc(sizeof(node_t));     // Declare variable
  node_t *temp_module=(node_t*)malloc(sizeof(node_t));      // Declare variable
  node_t *temp_module_last=(node_t*)malloc(sizeof(node_t)); // Declare variable
  temp_module->parent=temp_cutline;                         // Set temp_cutline as parent of temp_module
  temp_module->left=NULL;                                   // Set left as NULL
  temp_module->right=NULL;                                  // Set right as NULL
  temp_module->module=&modules[n];                          // Set module from array modules
  int i,init_displacement=0;                                // Declare variables
  for (i=0;i<n;i++){                                        // Loop through indices
    init_displacement+=modules[i].w;                        // Set init_displacement to sum of widths of modules
  }
  temp_module->module->llx=init_displacement;               // Set llx to init_displacement
  temp_module->cutline=UNDEFINED_CUTLINE;                   // Set cutline to UNDEFINED_CUTLINE

  temp_cutline->module=NULL;                                // Set module to NULL
  temp_cutline->cutline=V;                                  // Set cutline to V
  temp_cutline->parent=par;                                 // Set parent to parent
  temp_cutline->right=temp_module;                          // Set right to temp_module
  n++;                                                      // Increment index
  if (n<num_modules-1){                                     // Check if n has not reached end of array - 1
    temp_cutline->left=init_slicing_tree(temp_cutline,n);   // Save left to next module
  }
  else {                                                    // If last module
    temp_module_last->parent=temp_cutline;                  // Set parent to temp_cutline
    temp_module_last->left=NULL;                            // Set left to NULL
    temp_module_last->right=NULL;                           // Set left to NULL
    temp_module_last->module=&modules[n];                   // Set module from array modules
    for (i=0;i<n;i++){                                      // Loop through indices
      init_displacement+=modules[i].w;                      // Set init_displacement to sum of widths of modules
    }
    temp_module_last->module->llx=init_displacement;        // Set llx to init_displacement
    temp_module_last->cutline=UNDEFINED_CUTLINE;            // Set cutline to UNDEFINED_CUTLINE
    temp_cutline->left=temp_module_last;                    // Set to left of temp_cutline
  }
  return temp_cutline;                                      // Return temp_cutline
}
