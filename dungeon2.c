   ...
   /***************/
      ...
      while ( room < 4 )	/* Explore the maze */
      {
         if ( room == 0 )
         {
            ...
            printf("and a small door that can be entered by crouching.\n");		
            printf("Which exit do you wish to explore?\n");
            printf("Enter 0 for flagstone, 1 for air vent, or 2 for small door.  "); 
            scanf("%d", &choice); 
            if ( choice == 0 ) { room = 4; }
            if ( choice == 1 ) { room = 1; }
            if ( choice == 2 ) { room = 3; }
	    ...
	    printf("along passage.  "); 
         ...
         {
            printf("The small door leads to a new room ");
	    printf("with four exits.\n");
            printf("The first is a cave behind a boulder,\n ");
	    printf("the second is a passage small enough to crawl through,\n ");
	    printf("the third is a passage to the right,\n ");
	    printf("the fourth is a passage to the left.\n");
            printf("Enter 0 for cave, 1 for small passage, 2 for right, or 3 for left.  ");
            scanf("%d", &choice); 
            if ( choice == 0 ) { room = 0; }
            if ( choice == 1 ) { room = 4; }
	    if ( choice == 2 ) { room = 1; }
