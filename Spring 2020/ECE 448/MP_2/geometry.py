...
def computeCoordinate(start, length, angle):
    ...
    """
    x_coord = start[0] + int(length * math.cos(angle*math.pi/180))
    y_coord = start[1] - int(length * math.sin(angle*math.pi/180))
    return (x_coord, y_coord)

def distPointtoLine(armPosDist, objectCenter):
    """Compute distance from a given point to a line segment.
...
def doesArmTouchObjects(armPosDist, objects, isGoal=False):
    ...
    for arm in armPosDist:
        if isGoal == False:
            for obj in objects:
                if distPointtoLine(arm, (obj[0], obj[1])) - obj[2] - arm[2] <= 0:
                    return True
        else:
            for obj in objects:
                if distPointtoLine(arm, (obj[0], obj[1])) - obj[2] <= 0:
                    return True
    return False

def distPointtoPoint(start_pos, end_pos):
...
def doesArmTipTouchGoals(armEnd, goals):
    ...
    """
    for goal in goals:
        if distPointtoPoint(armEnd, goal) - goal[2] <= 0:
            return True
    return False
...
def isArmWithinWindow(armPos, window):
    ...
    for arm in armPos:
        ...
        if arm_begin[0] < 0 or arm_begin[0] > window[0] or arm_begin[1] < 0 or arm_begin[1] > window[1] or arm_end[0] < 0 or arm_end[0] > window[0] or arm_end[1] < 0 or arm_end[1] > window[1]:
            return False
