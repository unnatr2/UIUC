...
def blob_search(image, detector):
    ...
    # if len(keypoints) >= 1:
    #     print("Coordinates of blob 1 = ", (keypoints[0].pt[0], keypoints[0].pt[1]))
    ...
    for i in range (len(keypoints)):
        ...
        blob_image_center.append(coord)

    for i in range (len(keypoints)):
        new_pt = list(keypoints[i].pt)
        new_pt[0] += crop_top_col
        new_pt[1] += crop_top_row
        keypoints[i].pt = tuple(new_pt)
    ...
    pixel = [int(200), int(200)] # Change this to select a new dot location [column, row]
