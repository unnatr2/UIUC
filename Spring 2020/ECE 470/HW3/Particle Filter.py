...
# Set a time limit for simulation, will stop after 100 steps.
timsSpan = 100
num_rows = 10
num_cols = 16
grid_height = 25
...
class Maze(object):
    ...
    def show_maze(self):
        turtle.setworldcoordinates(
            0, 0, self.width * 1.005, self.height * 1.005)
        wally = turtle.Turtle()
        wally.speed(0)
        wally.width(1.5)
        wally.hideturtle()
...
class default_2D_Model:
    ...
    def readingMap(self, position):
        ...
        # (down, right, up, left)
        reading = self.map.distance_to_walls(
            (validPosition[0], validPosition[1]))
        return reading
    ...
    def simulateNextPosition(self, previousEstimate, currentControl=0):
        ...
        permissibilities = self.map.permissibilities(
            (cellY, cellX))  # (down, right, up, left)
    ...
    def run(self, currentControl=0):
        ...
        """
        cellX, cellY = int(
            self.x // self.grid_width), int(self.y // self.grid_height)
...
class particleFilter:
    ...
    def __init__(self, dimension=2, model=default_2D_Model(), numParticles=numberOfParticles, timeSpan=timsSpan, resamplingNoise=0.01, positionStd=5):
        ...
        for i in range(self.numParticles):
            x_pos = np.random.uniform(0, self.curMax[0])
            y_pos = np.random.uniform(0, self.curMax[1])
    ...
    def Sample_Motion_Model(self, u_t=0):
        ...
        for i in range(self.numParticles):
            self.particles[i] = self.model.simulateNextPosition(
                self.particles[i], u_t)
    ...
    def Measurement_Model(self):
        ...
        robot_pos = self.model.readingSensor()
        for i in range(self.numParticles):
            particle_pos = self.model.readingMap(self.particles[i])
            distance = 0
            for j in range(4):
                distance += (robot_pos[j] - particle_pos[j])**2
    ...
    def calcPosition(self):
        ...
        y_pos = 0
        for i in range(self.numParticles):
            x_pos += self.weights[i] * self.particles[i][0]
            y_pos += self.weights[i] * self.particles[i][1]

        if x_pos < self.curMin[0]:
            x_pos = self.curMin[0]
        elif x_pos > self.curMax[0]:
            x_pos = self.curMax[0]
        if y_pos < self.curMin[1]:
            y_pos = self.curMin[1]
        elif y_pos > self.curMax[1]:
            y_pos = self.curMax[1]
    ...
    #     """
    #     Create a list in increasing order from 0 to 1 using the
    #     normalized weight and randomly find an index in the list.
    #     Create new particle list based on the position of the indexed
    #     particle.
    #     """
    #     newParticles = []
    ...
    def resampling(self):
        ...
        # TODO: Comment the code below:
        """
        Create a list based on the weights of the particles with 
        higher weights being larger. The index is randomly found 
        and is more likely to land on a higher weight than on a 
        lower weight. Create new particle list based on the position 
        of the indexed particle
        """
        newParticles = []
        N = len(self.particles)
    ...
    def runParticleFilter(self):
        ...
        controls = [2, 3, 2, 2, 3, 2, 1, 2, 2, 2, 2, 2,
                    2, 3, 3, 2, 2, 3, 3, 3, 2, 3, 3, 2, 2, 3, 0]
        control = 0
