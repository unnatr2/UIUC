                                                   ...
                                                   queue<TreeNode *> &mergeQueue)
{
    ...
    {
        returnNode = mergeQueue.front();
        mergeQueue.pop();
        return returnNode;
    }
    else if (!singleQueue.empty() && mergeQueue.empty())
    {
        returnNode = singleQueue.front();
        singleQueue.pop();
        return returnNode;
    }
    else
...
void HuffmanTree::buildTree(const vector<Frequency> &frequencies)
{
    ...
    {
        TreeNode *newLeafNode = new TreeNode(frequencies[i]);
        singleQueue.push(newLeafNode);
    }
    while (singleQueue.size() + mergeQueue.size() >= 2)
    {
        TreeNode *leftNode;
        TreeNode *rightNode;
        leftNode = removeSmallest(singleQueue, mergeQueue);
        rightNode = removeSmallest(singleQueue, mergeQueue);
        int mergeNodeFreq = (leftNode->freq).getFrequency() + (rightNode->freq).getFrequency();
        TreeNode *mergeNode = new TreeNode(mergeNodeFreq);
...
HuffmanTree::TreeNode *HuffmanTree::readTree(BinaryFileReader &bfile)
{
    ...
    if (bfile.getNextBit())
    {
        Frequency leafFreq(bfile.getNextByte(), 0);
        TreeNode *newLeafNode = new TreeNode(leafFreq);
        return newLeafNode;
    }
    else
    {
        TreeNode *newInternalNode = new TreeNode(0);
        newInternalNode->left = readTree(bfile);
        newInternalNode->right = readTree(bfile);
