...
private:
  ...
  int partition(vector<Point<Dim>> &Points, int currentDim, int left, int right, int pivotIndex);
  void quickSelect(vector<Point<Dim>> &Points, int left, int right, int mid, int currentDim);
  KDTreeNode *makeTree(vector<Point<Dim>> &Points, int left, int right, int currentDim);
  ...
  KDTreeNode *downTree(const Point<Dim> &query, KDTreeNode *node, int currentDim, int &bottom, double &minDist, std::stack<KDTreeNode *> &parents, std::stack<bool> &direction) const;
  bool checkBounds(const Point<Dim> &query, const Point<Dim> &target, int currentDim, double &minDist) const;
  void upTree(const Point<Dim> &query, KDTreeNode *node, KDTreeNode *&currentBest, int currentDim, double &minDist, std::stack<KDTreeNode *> &parents, std::stack<bool> &direction) const;
  double nearestNeighbor(const Point<Dim> &query, KDTreeNode *node, KDTreeNode *&currentBest, int currentDim) const;
  double distance(const Point<Dim> &first, const Point<Dim> &second) const;
  // void _findNearestNeighbor(const Point<Dim> &query, KDTreeNode *const &node, int depth, KDTreeNode *&currentBest, double &minDist) const;
};
