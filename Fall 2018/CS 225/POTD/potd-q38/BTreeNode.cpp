...
{
    unsigned result = std::ceil(order / 2.0) - 1;
...
bool BTreeNode::rotateRight(unsigned idx, unsigned order)
{
    ...
    child->elements_.insert(child->elements_.begin(), elements_[idx - 1]);
    // child->elements_[0] = elements_[idx - 1];
    elements_[idx - 1] = sibling->elements_[sibling->elements_.size() - 1];
    sibling->elements_.pop_back();
