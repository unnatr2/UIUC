...
{
	const int left = (static_cast<int>(trailer->center().x()) - trailer->width() / 2) + WHEEL_RADIUS;
	const int right = (static_cast<int>(trailer->center().x()) + trailer->width() / 2) - WHEEL_RADIUS;
	const int y = (WHEEL_RADIUS * 1 / 2 + static_cast<int>(trailer->center().y()) + trailer->height() / 2);
